        ...
        private static void HandleIncomingMessage(string message)
        {
            ...
            if (message.StartsWith("freq"))
            {
                ...
                if (parts.Length == 2)
                {
                    ...
                    if (int.TryParse(parts[1], out var frequency) && frequency > 0 && frequency <= DeviceConfig.maxFreq)
                    {
            ...
            var response = message switch
            {
                ...
                "battery ?\r" => "Re:[] battery *capacity=21% *voltage=3.63V *current=-91.59mA *temperature=29.63celsius",
                // "freq 50\r" => "Re:[] ok",

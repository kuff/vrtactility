...
namespace Tactility.Calibration
{
    ...
    public class TactilityDeviceConfig : ScriptableObject
    {
        ...
        public int[] anodes;
        [Tooltip("The remapped indexes of the pads. This is used to determine the order in which pads are addressed in the command string. If this field is not set, the default behaviour is to use the pads in the order they are declared in the command string. If this field is set, the values in this field are used to determine the order in which pads are addressed. For example, if the device has 4 pads and the mapping is { 0, 1, 2, 3 }, then the pads are addressed in the order 0, 1, 2, 3. If the device has 4 pads and the mapping is { 3, 2, 1, 0 }, then the pads are addressed in the order 3, 2, 1, 0. If this field is set, the length of the array must be equal to the number of pads. If this field is set, the values in this field must be unique. If this field is set, the values in this field must be in the range [0, numPads - 1]")]
        public int[] mapping;
